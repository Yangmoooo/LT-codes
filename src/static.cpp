...
Data Decode(uint8_t *encode_data_ptr, uint32_t encode_data_size,
    ...
    while (flag) {
        flag = false;
        for (uint32_t i = 0; i < packet_cnt; ++i) {
            uint8_t crc = CalcCrc(encode_data_ptr + i * packet_size,
                                  block_size + kSeedSize);
            if (crc !=
                encode_data_ptr[i * packet_size + block_size + kSeedSize]) {
                continue;
            }
            uint32_t seed =
            ...
            if (degree == 1) {
                ...
                memcpy(block_ptr, encode_data_ptr + i * packet_size,
                       block_size);
