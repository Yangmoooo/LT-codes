...
Data encode(uint8_t *real_data_ptr, uint32_t real_data_size,
  ...
  uint8_t *write_data_ptr =
      (uint8_t *)calloc(packet_size * packet_cnt, sizeof(uint8_t));
  if (!write_data_ptr) {
  ...
  for (uint32_t i = 0; i < packet_cnt; ++i) {
    ...
    std::unordered_set<uint32_t> indexes = gen_indexes(seed, degree, block_cnt);
    ...
    write_data_tmp_ptr[block_size + seed_size] =
        calc_crc(write_data_tmp_ptr, block_size + seed_size);
  }
  Data write_data = {
      .ptr = write_data_ptr,
...
Data decode(uint8_t *encode_data_ptr, uint32_t encode_data_size,
  ...
  while (flag) {
    ...
    for (uint32_t i = 0; i < packet_cnt; ++i) {
      uint8_t crc =
          calc_crc(encode_data_ptr + i * packet_size, block_size + seed_size);
