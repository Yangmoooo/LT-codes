...
uint32_t get_seed_type(SeedHeader& header) {
    ...
    if (error_pos) {
        switch (error_pos) {
            case 1: header.p1 = !header.p1; break;
            case 2: header.p2 = !header.p2; break;
            case 3: header.d1 = !header.d1; break;
            case 4: header.p3 = !header.p3; break;
            case 5: header.d2 = !header.d2; break;
            default: break;
        }
    }
...
Data encode(uint8_t* real_data_ptr, uint32_t real_data_size, uint32_t block_size,
    ...
    for (uint32_t i = 0; i < 4; ++i) {
        for (uint32_t j = 0; j < packet_cnts[i]; ++j) {
            ...
            // 构造 seed
            SeedHeader seed_header = {.d1 = static_cast<uint8_t>(i >> 1),
